#!/usr/bin/env bash
# Run the IT-support-triage structured-output demo using the project's local venv.
# Loads .env (via llm_config.py -> python-dotenv), then calls the OpenAI SDK
# through the bifrost proxy with response_format=json_object.
...
exec "$VENV_PY" "$HERE/it_support_triage.py" "$@"
